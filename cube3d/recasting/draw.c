...
void	draw_tile(t_mlx *mlx, int x, int y, int color)
{
	...
	i = 0;
	while (i < mlx->range_ve_size)
	{
		j = 0;
		while (j < mlx->range_ho_size)
		{
			my_mlx_pixel_put(mlx, x * mlx->range_ho_size + j,
				y * mlx->range_ve_size + i, color);
			j++;
		}
		i++;
	}
...
void	set_player_position(t_mlx *mlx, int x, int y)
{
	if (mlx->flag == 0)
	{
	...
	while (map->mp_arrs[y])
	{
		...
		while (map->mp_arrs[y][x])
		{
			if (map->mp_arrs[y][x] == '1')
				draw_tile(mlx, x, y, 0x000000);
			else
			{
				draw_tile(mlx, x, y, 0xFFFFFF);
				if (ft_strchr("WESN", map->mp_arrs[y][x]))
					set_player_position(mlx, x, y);
			}
			x++;
		}
		y++;
